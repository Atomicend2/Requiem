                  ...
                  return (
                    <div key={bot.id} className="glass-card rounded-xl px-5 py-5 border border-white/5 hover:border-teal-400/20 transition-colors space-y-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            ...
                            <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded border border-violet-400/40 text-violet-300 bg-violet-400/10 font-bold">
                      ...
                      {!bot.pairingCode && bot.status !== "connected" && (
                        <div className="bg-rose-400/5 border border-rose-400/20 rounded-lg px-4 py-3 space-y-2">
                          <p className="text-[10px] uppercase tracking-widest text-rose-400/70">Request Pairing Code</p>
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-1 px-3 py-1.5 bg-black/30 border border-white/10 rounded-lg text-white placeholder:text-muted-foreground focus:outline-none focus:border-rose-400/50 text-xs font-mono"
                            />
                            <button
                              onClick={() => requestPairingCode(bot.id)}
                              disabled={pairingLoading[bot.id] || !pairingPhones[bot.id]?.trim()}
                              className="px-3 py-1.5 rounded-lg bg-rose-400/10 border border-rose-400/30 text-rose-400 text-xs font-bold uppercase tracking-wider hover:bg-rose-400/20 disabled:opacity-50 transition-colors whitespace-nowrap">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={() => toggleBotRole(bot, "otp")} disabled={actionPending}
                          className={cn(
                            ...
                            hasOtp ? "bg-rose-400/10 border-rose-400/30 text-rose-400 hover:bg-rose-400/20" : "bg-white/5 border-white/10 text-muted-foreground hover:border-rose-400/30 hover:text-rose-400"
                          )}>
...
function CardImportPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* ── Import Panel ───────────────────────────────────────────────── */}
      <section className="glass-card rounded-xl p-6 border border-rose-500/20">
        <h2 className="font-serif text-xl font-bold text-white mb-1 flex items-center gap-2">
          <Download className="w-5 h-5 text-rose-400" /> Import Cards — Shoob.gg
        </h2>
        <p className="text-muted-foreground text-sm mb-6">
          Fetch card data directly from Shoob.gg. Images are downloaded and stored locally in the database.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
            ...
            <>
              <div>
                ...
                <input type="text" value={anime} onChange={(e) => setAnime(e.target.value)} placeholder="e.g. Naruto, One Piece"
                  className="w-full px-3 py-2.5 bg-black/30 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-rose-400/50 placeholder:text-muted-foreground" />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-muted-foreground mb-1.5 block">Tier (optional)</label>
                <select value={tier} onChange={(e) => setTier(e.target.value)}
                  className="w-full px-3 py-2.5 bg-black/30 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-rose-400/50">
          ...
          {(importMode === "sync" || importMode === "full") && (
            <div className="col-span-full bg-black/20 rounded-lg p-4 border border-violet-500/20 text-sm text-muted-foreground">
              {importMode === "sync"
        ...
        <button onClick={handleImport} disabled={loading || (importMode === "anime" && !anime.trim())}
          className="px-6 py-2.5 rounded-lg bg-rose-500/20 border border-rose-400/40 text-rose-300 font-bold uppercase tracking-widest text-sm hover:bg-rose-500/30 transition-colors disabled:opacity-50 flex items-center gap-2">
      ...
      <section className="glass-card rounded-xl p-6 border border-violet-500/20">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-serif text-xl font-bold text-white flex items-center gap-2">
            <RefreshCw className="w-5 h-5 text-violet-400" /> Shoob Sync Status
          </h2>
          <div className="flex gap-2">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              {scraperStatus.total_cards !== undefined && (
                <div className="bg-black/20 rounded-lg p-3 border border-white/5 text-center">
                  ...
                  <p className="text-sm font-bold text-rose-400">{scraperStatus.total_cards?.toLocaleString()}</p>
                </div>
              )}
              {scraperStatus.shoob_cards !== undefined && (
                <div className="bg-black/20 rounded-lg p-3 border border-white/5 text-center">
                  ...
                  <p className="text-sm font-bold text-violet-400">{scraperStatus.shoob_cards?.toLocaleString()}</p>
                </div>
              )}
            ...
            {scraperStatus.tracked_ids !== undefined && (
              <div className="text-xs text-muted-foreground bg-black/20 rounded p-3 border border-white/5">
                ...
                {scraperStatus.last_run_type && <span className="ml-4">Last run type: <strong className="text-violet-400">{scraperStatus.last_run_type}</strong></span>}
              </div>
            )}
          </div>
        ) : (
