...
export default function World() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col" style={{ background: "linear-gradient(180deg,#08030c 0%,#150a1f 35%,#241133 60%,#0c0512 100%)" }}>

      {/* ── Sky background layers ── */}
      <div className="absolute inset-0 pointer-events-none select-none z-0">
        {/* Celestial glow */}
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 90% 55% at 50% 0%, rgba(225,29,72,0.18) 0%, transparent 70%)" }} />
        <div className="absolute" style={{ left:"15%", top:"5%", width:500, height:500, background:"radial-gradient(circle, rgba(147,51,234,0.10) 0%, transparent 70%)", borderRadius:"50%", filter:"blur(40px)" }} />
        <div className="absolute" style={{ right:"8%", top:"8%", width:340, height:340, background:"radial-gradient(circle, rgba(124,58,237,0.12) 0%, transparent 70%)", borderRadius:"50%", filter:"blur(30px)" }} />
        <div className="absolute" style={{ left:"35%", bottom:"15%", width:600, height:200, background:"radial-gradient(ellipse, rgba(225,29,72,0.06) 0%, transparent 70%)", borderRadius:"50%", filter:"blur(20px)" }} />
        ...
        <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 1280 720" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
          ...
          <ellipse cx="200"  cy="140" rx="280" ry="45" fill="rgba(216,180,254,0.5)" filter="url(#cloud-blur)" />
          <ellipse cx="900"  cy="80"  rx="200" ry="30" fill="rgba(216,180,254,0.4)" filter="url(#cloud-blur)" />
          <ellipse cx="640"  cy="320" rx="350" ry="38" fill="rgba(216,180,254,0.25)" filter="url(#cloud-blur)" />
          <ellipse cx="1100" cy="460" rx="220" ry="30" fill="rgba(216,180,254,0.2)" filter="url(#cloud-blur)" />
          <ellipse cx="300"  cy="550" rx="260" ry="36" fill="rgba(216,180,254,0.18)" filter="url(#cloud-blur)" />
        </svg>
        ...
        <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 100 100" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
          <line x1="50" y1="38" x2="22" y2="65" stroke="#e11d48" strokeWidth="0.15" strokeDasharray="1 2" />
          <line x1="50" y1="38" x2="78" y2="22" stroke="#e11d48" strokeWidth="0.15" strokeDasharray="1 2" />
          <line x1="50" y1="38" x2="36" y2="28" stroke="#e11d48" strokeWidth="0.15" strokeDasharray="1 2" />
          <line x1="50" y1="38" x2="68" y2="75" stroke="#e11d48" strokeWidth="0.15" strokeDasharray="1 2" />
          <line x1="22" y1="65" x2="68" y2="75" stroke="#e11d48" strokeWidth="0.10" strokeDasharray="1 3" />
          <line x1="36" y1="28" x2="78" y2="22" stroke="#e11d48" strokeWidth="0.10" strokeDasharray="1 3" />
        </svg>
        ...
        <svg className="absolute inset-0 w-full h-full opacity-[0.06]" viewBox="0 0 100 100" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
          {Array.from({ length: 9 }, (_, i) => (
            <line key={`h${i}`} x1="0" y1={`${(i+1)*10}`} x2="100" y2={`${(i+1)*10}`} stroke="#e11d48" strokeWidth="0.3" />
          ))}
          {Array.from({ length: 11 }, (_, i) => (
            <line key={`v${i}`} x1={`${i*10}`} y1="0" x2={`${i*10}`} y2="100" stroke="#e11d48" strokeWidth="0.3" />
          ))}
          {/* Concentric circles from centre */}
          {[10,20,32,45].map(r => (
            <circle key={r} cx="50" cy="38" r={r} fill="none" stroke="#e11d48" strokeWidth="0.2" strokeDasharray="2 4" />
          ))}
        </svg>
        ...
        <svg className="absolute opacity-10" style={{ bottom:80, left:32, width:80, height:80 }} viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
          <circle cx="40" cy="40" r="38" fill="none" stroke="#e11d48" strokeWidth="0.8" />
          <circle cx="40" cy="40" r="30" fill="none" stroke="#e11d48" strokeWidth="0.4" />
          <line x1="40" y1="2"  x2="40" y2="78" stroke="#e11d48" strokeWidth="0.6" />
          <line x1="2"  y1="40" x2="78" y2="40" stroke="#e11d48" strokeWidth="0.6" />
          <polygon points="40,5 43,38 40,33 37,38" fill="#e11d48" opacity="0.7" />
          <text x="40" y="16"  textAnchor="middle" fill="#e11d48" fontSize="7" fontFamily="monospace">N</text>
          <text x="40" y="70"  textAnchor="middle" fill="#e11d48" fontSize="7" fontFamily="monospace">S</text>
          <text x="69" y="43"  textAnchor="middle" fill="#e11d48" fontSize="7" fontFamily="monospace">E</text>
          <text x="12" y="43"  textAnchor="middle" fill="#e11d48" fontSize="7" fontFamily="monospace">W</text>
        </svg>
      </div>
      ...
      <div className="relative z-20 p-6 md:p-8 pointer-events-none" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.65), transparent)" }}>
        <p className="font-mono tracking-[0.5em] text-xs uppercase mb-1" style={{ color:"rgba(225,29,72,0.4)" }}>反逆</p>
        ...
        <p className="mt-2 max-w-xl text-sm" style={{ color:"rgba(216,180,254,0.5)" }}>
          ...
          >
            <div className="relative">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-68 p-4 rounded-xl opacity-0 translate-y-2 pointer-events-none group-hover/marker:opacity-100 group-hover/marker:translate-y-0 transition-all duration-300 z-50"
                style={{ width:260, background:"rgba(22,9,26,0.92)", border:"1px solid rgba(225,29,72,0.18)", boxShadow:"0 0 30px rgba(225,29,72,0.2)" }}>
                ...
                <p className="text-xs leading-relaxed" style={{ color:"rgba(216,180,254,0.55)" }}>{region.desc}</p>
                <div className="mt-3 pt-2 text-[10px] font-bold tracking-[0.2em] uppercase text-center" style={{ borderTop:"1px solid rgba(255,255,255,0.05)", color:"rgba(225,29,72,0.6)" }}>
      ...
      {/* ── Legend ── */}
      <div className="absolute z-20 p-4 rounded-xl" style={{ bottom:80, right:24, background:"rgba(22,9,26,0.85)", border:"1px solid rgba(225,29,72,0.15)", boxShadow:"0 0 20px rgba(225,29,72,0.08)" }}>
        <h4 className="text-[10px] font-mono font-bold tracking-[0.3em] uppercase pb-2 mb-3" style={{ color:"rgba(225,29,72,0.5)", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
          Map Legend
        </h4>
        <ul className="space-y-2 text-xs text-white/70">
          {[
            { dot:"bg-amber-400 shadow-[0_0_6px_rgba(251,191,36,0.8)]",  label:"Capital" },
            { dot:"bg-primary shadow-[0_0_6px_rgba(225,29,72,0.8)]",   label:"Friendly / HQ" },
      ...
      {/* ── Coord label (flavour) ── */}
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-20 font-mono text-[10px] tracking-widest" style={{ color:"rgba(225,29,72,0.25)" }}>
